...
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-8 px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        ...
                        <?php
                        $topCategory = \App\Models\TrainingCategory::withCount('subcategories')
                            ->orderBy('subcategories_count', 'desc')
                            ->first();
                        ?>
                        <p class="text-3xl font-bold text-slate-800"><?php echo e($topCategory->subcategories_count ?? 0); ?></p>
                        <p class="text-xs text-slate-500 mt-1 truncate"><?php echo e($topCategory->title ?? 'Belum ada'); ?></p>
                    </div>
                        ...
                        <tr class="hover:bg-slate-50 transition">
                            ...
                            <td class="px-4 py-4">
                                ...
                                <div class="relative">
                                    <button
                                        type="button"
                                        onclick="trainToggleDropdown(<?php echo e($index); ?>)"
                            ...
                            <td class="px-4 py-4 whitespace-nowrap">
                                <div class="flex items-center justify-center gap-1">
                                    <button onclick='trainOpenEditModal(<?php echo json_encode($training, 15, 512) ?>)' class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </button>
                                    <button onclick='trainOpenDeleteModal(<?php echo json_encode($training, 15, 512) ?>)' class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition" title="Hapus">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full my-8 transform transition-all max-h-[90vh] flex flex-col">
            ...
            <form class="p-6 space-y-6 overflow-y-auto flex-1" method="POST" action="<?php echo e(route('training.store')); ?>">
                ...
                <div>
                    ...
                    <input type="text" name="provider" required placeholder="Contoh: AOSH, PT Safety Pro" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                ...
                <div>
                    ...
                    <select name="status" required class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                        <option value="active">Active</option>
                        <option value="inactive">Inactive</option>
                    </select>
                ...
                <div>
                    ...
                    <textarea name="provider_description" rows="3" placeholder="Deskripsi singkat tentang program atau program ini" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none"></textarea>
                </div>
                    ...
                    <div id="trainingInputs" class="space-y-3 max-h-[40vh] overflow-y-auto pr-2">
                        <div class="flex flex-col gap-2 p-4 bg-slate-50 rounded-lg border border-slate-200">
                            <div class="flex items-start gap-2">
                                <input type="text" name="trainings[0][name]" required placeholder="Nama pelatihan (contoh: Basic Safety Induction)" class="flex-1 px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm">
                                <select name="trainings[0][status]" class="w-28 px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm">
                                    <option value="active">Active</option>
                                    <option value="inactive">Inactive</option>
                                </select>
                                <button type="button" onclick="trainRemoveTrainingInput(this)" class="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                    </svg>
                                </button>
                            </div>
                            <textarea name="trainings[0][description]" rows="2" placeholder="Deskripsi pelatihan ini (opsional)" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none text-sm"></textarea>
                        </div>
                    </div>
                    <button type="button" onclick="trainAddTrainingInput()" class="mt-3 inline-flex items-center px-4 py-2 bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 transition font-medium">
    ...
    <div id="editModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 overflow-y-auto">
        <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full my-8 transform transition-all max-h-[90vh] flex flex-col">
            ...
            <form id="editForm" class="p-6 space-y-6 overflow-y-auto flex-1" method="POST">
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Nama Program *</label>
                    <input type="text" id="editProvider" name="provider" required placeholder="Contoh: AOSH, PT Safety Pro" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent">
                    <p class="mt-1 text-xs text-slate-500">Masukkan nama program atau lembaga penyelenggara pelatihan</p>
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Status Program *</label>
                    <select id="editProviderStatus" name="status" required class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent">
                        <option value="active">Active</option>
                        <option value="inactive">Inactive</option>
                    </select>
                    <p class="mt-1 text-xs text-slate-500">Status aktifitas program ini</p>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Deskripsi Program</label>
                    <textarea id="editProviderDescription" name="provider_description" rows="3" placeholder="Deskripsi singkat tentang program atau provider ini" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent resize-none"></textarea>
                ...
                <div>
                    ...
                    <div id="editTrainingInputs" class="space-y-3 max-h-[40vh] overflow-y-auto pr-2"></div>
                    <button type="button" onclick="trainAddEditTrainingInput()" class="mt-3 inline-flex items-center px-4 py-2 bg-amber-100 text-amber-700 rounded-lg hover:bg-amber-200 transition font-medium">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                        </svg>
                        Tambah Pelatihan
    ...
    <script>
function trainAddTrainingInput() {
    ...
    newInput.innerHTML = `
        <div class="flex items-start gap-2">
            <input type="text" name="trainings[${index}][name]" required placeholder="Nama pelatihan" class="flex-1 px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm">
            <select name="trainings[${index}][status]" class="w-28 px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm">
                <option value="active">Active</option>
                <option value="inactive">Inactive</option>
            </select>
            <button type="button" onclick="trainRemoveTrainingInput(this)" class="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition">
        ...
        <textarea name="trainings[${index}][description]" rows="2" placeholder="Deskripsi pelatihan (opsional)" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none text-sm"></textarea>
    `;
...
function trainOpenEditModal(data) {
    ...
    if (data.trainings && Array.isArray(data.trainings)) {
        data.trainings.forEach((training, index) => {
            ...
            newInput.innerHTML = `
                <div class="flex items-start gap-2">
                    <input type="text" name="trainings[${index}][name]" required value="${(training.name || '').replace(/"/g, '&quot;')}" placeholder="Nama pelatihan" class="flex-1 px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm">
                    <select name="trainings[${index}][status]" class="w-28 px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm">
                    ...
                    <button type="button" onclick="trainRemoveEditTrainingInput(this)" class="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
                </div>
                <textarea name="trainings[${index}][description]" rows="2" placeholder="Deskripsi pelatihan (opsional)" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent resize-none text-sm">${(training.description || '').replace(/"/g, '&quot;')}</textarea>
            `;
            container.appendChild(newInput);
        });
    }
...
function trainAddEditTrainingInput() {
    const container = document.getElementById('editTrainingInputs');
    const index = container.children.length;
    const newInput = document.createElement('div');
    newInput.className = 'flex flex-col gap-2 p-4 bg-slate-50 rounded-lg border border-slate-200';
    newInput.innerHTML = `
        <div class="flex items-start gap-2">
            <input type="text" name="trainings[${index}][name]" required placeholder="Nama pelatihan" class="flex-1 px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm">
            <select name="trainings[${index}][status]" class="w-28 px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm">
                <option value="active">Active</option>
                <option value="inactive">Inactive</option>
            </select>
        ...
        <textarea name="trainings[${index}][description]" rows="2" placeholder="Deskripsi pelatihan (opsional)" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent resize-none text-sm"></textarea>
    `;
    container.appendChild(newInput);
}
...
function trainToggleDropdown(index) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function trainFilterTable() {
        ...
        tableRows.forEach(row => {
            ...
            const matchesSearch = !searchTerm || provider.includes(searchTerm) || training.includes(searchTerm);
    ...
    providerFilter.addEventListener('change', trainFilterTable);
    statusFilter.addEventListener('change', trainFilterTable);
});
    </script>
</div>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.adminmain', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\indo_drilling_school\resources\views/training.blade.php ENDPATH**/ ?>