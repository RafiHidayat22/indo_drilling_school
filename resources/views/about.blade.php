...
@section('title', 'About | Indonesia Drilling School')
...
<section class="py-20 bg-slate-900 text-white relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            @foreach ($facilities as $index => [$title, $icon, $desc])
            <div class="group" data-aos="zoom-in" data-aos-delay="{{ $index * 50 }}">
...
<section class="py-20 bg-gradient-to-b from-white to-gray-50 px-6">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            @foreach ($advantages as $index => [$title, $icon, $desc, $color])
            <div class="group relative" data-aos="fade-up" data-aos-delay="{{ $index * 100 }}">
                <!-- Overlay gradient untuk hover effect -->
                <div class="absolute inset-0 bg-gradient-to-br from-{{ $color }}-600 to-{{ $color }}-800 rounded-2xl opacity-0 group-hover:opacity-100 transition duration-300 transform group-hover:scale-105"></div>
                <!-- Kartu utama -->
                <div class="relative bg-white border-2 border-gray-100 rounded-2xl p-8 hover:border-{{ $color }}-500 transition duration-300 h-full">
                    <!-- Ikon -->
                    <div class="w-16 h-16 bg-gradient-to-br from-{{ $color }}-100 to-{{ $color }}-200 rounded-2xl flex items-center justify-center mb-6 group-hover:bg-white/20 transition duration-300">
                        <i class="fa-solid {{ $icon }} text-3xl text-{{ $color }}-600 group-hover:text-{{ $color }}-700 transition duration-300"></i> <!-- Ubah warna ikon saat hover -->
                    </div>
                    <!-- Judul dan Deskripsi -->
                    <h3 class="text-xl font-bold text-slate-900 group-hover:text-slate-900 mb-3 transition duration-300">{{ $title }}</h3> <!-- Ubah group-hover:text-white menjadi group-hover:text-slate-900 -->
                    <p class="text-gray-600 group-hover:text-gray-800 transition duration-300">{{ $desc }}</p> <!-- Ubah group-hover:text-white/90 menjadi group-hover:text-gray-800 -->
                </div>
            </div>
            @endforeach
        </div>
