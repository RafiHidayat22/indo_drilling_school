    ...
    <div class="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto mt-12 px-4" data-aos="fade-up" data-aos-delay="400">
            <div class="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 hover:bg-white/15 transition duration-300">
                <div class="text-3xl font-bold text-white mb-1 counter-number" data-count="{{ $stats['total_articles'] }}">0</div>
                <div class="text-sm text-gray-300">Articles</div>
            </div>
            <div class="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 hover:bg-white/15 transition duration-300">
                <div class="text-3xl font-bold text-white mb-1 counter-number" data-count="{{ $stats['categories'] }}">0</div>
                <div class="text-sm text-gray-300">Categories</div>
            </div>
            <div class="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 hover:bg-white/15 transition duration-300">
                <div class="text-3xl font-bold text-white mb-1 counter-number" data-count="{{ $stats['total_views'] }}">0</div>
            ...
            <div class="lg:col-span-2">
                <div class="grid md:grid-cols-2 gap-8" id="articlesGrid">
                    ...
                    <div class="col-span-2 text-center py-12">
                        <div class="inline-block p-8 bg-gray-50 rounded-2xl">
                            ...
                            <a href="{{ route('articles.index') }}" class="inline-block mt-4 px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition">
                                Clear Filters
                            </a>
                        </div>
                    </div>
                ...
                <div class="flex justify-center items-center gap-2 mt-12" data-aos="fade-up">
                    <div class="flex items-center gap-2">
                        ...
                        @else
                            <a href="{{ $articles->previousPageUrl() }}" class="w-10 h-10 rounded-lg border-2 border-gray-300 flex items-center justify-center hover:border-red-600 hover:text-red-600 transition duration-300">
                                <i class="fa-solid fa-chevron-left"></i>
                            </a>
                        @endif
                        ...
                        @foreach(range(1, $articles->lastPage()) as $page)
                            @if($page == $articles->currentPage())
                                <button class="w-10 h-10 rounded-lg bg-red-600 text-white font-bold">{{ $page }}</button>
                            @elseif($page == 1 || $page == $articles->lastPage() || abs($page - $articles->currentPage()) <= 2)
                                <a href="{{ $articles->url($page) }}" class="w-10 h-10 rounded-lg border-2 border-gray-300 flex items-center justify-center hover:border-red-600 hover:text-red-600 transition duration-300">
                                    {{ $page }}
                                </a>
                        ...
                        @if ($articles->hasMorePages())
                            <a href="{{ $articles->nextPageUrl() }}" class="w-10 h-10 rounded-lg border-2 border-gray-300 flex items-center justify-center hover:border-red-600 hover:text-red-600 transition duration-300">
                        ...
                        <ul class="space-y-3">
                            ...
                            <li>
                                ...
                                <a href="{{ route('articles.index', ['category' => $category->slug]) }}" 
                                   class="flex items-center justify-between p-3 rounded-xl {{ $bgClass }} transition duration-300 group">
                                    <span class="flex items-center gap-2 text-gray-700 {{ $textClass }} font-medium">
                                        <i class="{{'fa-solid fa-folder' }}"></i>
                                        {{ $category->name }}
                                    </span>
                                    <span class="text-sm {{ $badgeBgClass }} {{ $badgeTextClass }} px-3 py-1 rounded-full font-bold">
                        ...
                        <div class="space-y-4">
                            ...
                            <a href="{{ route('articles.show', $recent->slug) }}" class="flex gap-4 group">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-gray-400 text-sm">{{ $recent->formatted_date }}</p>
                                </div>
                            </a>
                            @endforeach
                        </div>
...
<section class="py-20 bg-gradient-to-br from-slate-900 via-red-900 to-slate-900 relative overflow-hidden">
    ...
    <div class="max-w-5xl mx-auto px-6 text-center relative z-10" data-aos="zoom-in">
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="{{ route('program.index') }}" class="inline-flex items-center justify-center gap-2 px-8 py-4 bg-gradient-to-r from-red-600 to-red-700 text-white font-bold rounded-xl hover:shadow-2xl hover:shadow-red-600/50 transition-all duration-300 transform hover:scale-105">
                Explore Programs
                <i class="fa-solid fa-arrow-right"></i>
            </a>
