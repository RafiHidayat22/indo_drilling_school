    ...
    <div class="max-w-7xl mx-auto">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        ...
                        @endphp
                        <p class="text-3xl font-bold text-slate-800">{{ $totalSubcategories }}</p>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        ...
                        ->first();
                        @endphp
                        <p class="text-3xl font-bold text-slate-800">{{ $topCategory->subcategories_count ?? 0 }}</p>
                        <p class="text-xs text-slate-500 mt-1 truncate" title="{{ $topCategory->title ?? 'Belum ada' }}">
                            {{ $topCategory->title ?? 'Belum ada' }}
                        </p>
                    </div>
            ...
            <div class="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white">
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-sm text-slate-600">
                        Total: <span class="font-semibold text-purple-600">{{ $trainings->total() }}</span> program
                    </div>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200">
                    ...
                    <tbody class="bg-white divide-y divide-slate-200">
                        ...
                        @forelse($groupedByCategory as $categoryName => $categoryTrainings)
                        @php $categoryIndex++; @endphp

                        @if(count($categoryTrainings) === 0)
                        {{-- Empty Category Row --}}
                        <tr class="hover:bg-slate-50 transition filterable-row"
                            data-category="{{ strtolower($categoryName) }}"
                            data-category-name="{{ $categoryName }}"
                            data-category-original-index="{{ $categoryIndex }}"
                            data-provider=""
                            data-status="empty"
                            ...
                            <td colspan="6" class="px-4 py-8">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-xs text-slate-500 mb-3">Silakan tambahkan program untuk kategori "{{ $categoryName }}"</p>
                                    <button
                                        onclick="trainOpenAddModalWithCategory('{{ $categoryName }}')"
                        ...
                        @else
                        {{-- Normal Category Rows with Programs --}}
                        @foreach($categoryTrainings as $subIndex => $training)
                        <tr class="hover:bg-slate-50 transition filterable-row {{ $subIndex > 0 ? 'border-t-0' : '' }}"
                            data-category="{{ strtolower($training['category'] ?? '') }}"
                            data-category-name="{{ $training['category'] ?? '' }}"
                            data-category-original-index="{{ $categoryIndex }}"
                            data-provider="{{ strtolower($training['provider'] ?? '') }}"
                            data-status="{{ strtolower($training['status'] ?? 'active') }}"
                            data-is-first-row="{{ $subIndex === 0 ? 'true' : 'false' }}"
                            data-is-empty="false"
                            data-category-programs-count="{{ count($categoryTrainings) }}">

                            <!-- No - Hanya tampil di baris pertama kategori -->
                            <td class="px-4 py-4 whitespace-nowrap text-sm text-slate-700 font-medium category-number-cell {{ $subIndex > 0 ? 'border-t-0' : '' }}">
                                @if($subIndex === 0)
                                <div class="text-black font-bold text-sm">
                                    {{ $categoryIndex }}
                                </div>
                            ...
                            <td class="px-4 py-4 whitespace-nowrap category-name-cell {{ $subIndex > 0 ? 'border-t-0' : '' }}">
                                @if($subIndex === 0)
                                <div class="flex items-center gap-2">
                                    ...
                                    <div>
                                        <div class="text-sm font-semibold text-slate-900">{{ $categoryName }}</div>
                                        <div class="text-xs text-slate-500">{{ count($categoryTrainings) }} program</div>
                                    </div>
                                </div>
                                @endif
                            </td>
                            ...
                            <!-- Program -->
                            <td class="px-4 py-4 {{ $subIndex > 0 ? 'border-t border-dashed border-slate-200' : '' }}">
                                <div class="flex items-start gap-3">
                                    <div class="flex-shrink-0 h-10 w-10 bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg flex items-center justify-center shadow-md">
                                        <span class="text-white font-bold text-sm">{{ strtoupper(substr($training['provider'], 0, 2)) }}</span>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <div class="text-sm font-bold text-slate-900 mb-0.5">{{ $training['provider'] }}</div>
                                        @if(!empty($training['provider_description']))
                                        <div class="text-xs text-slate-600 line-clamp-2">{{ $training['provider_description'] }}</div>
                                        @else
                            ...
                            <td class="px-4 py-4 {{ $subIndex > 0 ? 'border-t border-dashed border-slate-200' : '' }}">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <button
                                        type="button"
                                        onclick="trainToggleDropdown('{{ $training['id'] }}')"
                                        ...
                                        <div class="flex items-center gap-3">
                                            <div class="h-8 w-8 rounded-full bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center flex-shrink-0 shadow-sm">
                                                <span class="text-white font-bold text-sm">{{ count($training['trainings']) }}</span>
                                            </div>
                                            <div class="text-left">
                                                <div class="text-sm font-bold text-slate-900">{{ count($training['trainings']) }} Pelatihan</div>
                                                <div class="text-xs text-slate-600">
                                                    {{ collect($training['trainings'])->where('status', 'active')->count() }} aktif,
                                                    {{ collect($training['trainings'])->where('status', 'inactive')->count() }} inactive
                                                </div>
                                            </div>
                                        </div>
                                        <svg
                                            id="dropdown-icon-{{ $training['id'] }}"
                                    ...
                                    <div
                                        ...
                                        <div class="px-4 py-2.5 bg-slate-50 border-t border-slate-200">
                                            <div class="flex items-center justify-between text-xs text-slate-600">
                                                ...
                                                <span class="font-semibold text-purple-600">{{ count($training['trainings']) }} items</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <!-- Status -->
                            <td class="px-4 py-4 whitespace-nowrap text-center {{ $subIndex > 0 ? 'border-t border-dashed border-slate-200' : '' }}">
                            ...
                            <td class="px-4 py-4 whitespace-nowrap text-center {{ $subIndex > 0 ? 'border-t border-dashed border-slate-200' : '' }}">
                                <div class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 rounded-lg border border-slate-200">
                                    ...
                                    <span class="text-sm font-semibold text-slate-600">
                                        {{ count($training['trainings'] ?? []) }}
                                    </span>
                                </div>
                            </td>

                            <!-- Tanggal -->
                            <td class="px-4 py-4 whitespace-nowrap {{ $subIndex > 0 ? 'border-t border-dashed border-slate-200' : '' }}">
                                <div class="text-sm font-medium text-slate-900">
                                    {{ \Carbon\Carbon::parse($training['created_at'])->format('d M Y') }}
                                </div>
                                <div class="text-xs text-slate-500">
                                    {{ \Carbon\Carbon::parse($training['created_at'])->format('H:i') }} WIB
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-4 whitespace-nowrap {{ $subIndex > 0 ? 'border-t border-dashed border-slate-200' : '' }}">
                                <div class="flex items-center justify-center gap-1">
                                    <button
                                        onclick='trainOpenEditModal(@json($training))'
                                    ...
                                    <button
                                        onclick='trainOpenDeleteModal(@json($training))'
        ...
        <div id="addModal" class="hidden fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
            <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full my-8 transform transition-all max-h-[90vh] flex flex-col">
                ...
                <form class="p-6 space-y-6 overflow-y-auto flex-1" method="POST" action="{{ route('training.store') }}">
                    @csrf
                    <div>
