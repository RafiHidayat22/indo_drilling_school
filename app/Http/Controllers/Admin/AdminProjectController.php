...
class AdminProjectController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Order *after* filtering, dan handle NULL order properly
        $projects = $query->orderByRaw('COALESCE(`order`, 0) DESC')
            ->orderBy('created_at', 'desc')
            ->paginate(10)
            ->withQueryString(); // 🔑 ini wajib — biar pagination bawa query filter

        // Stats (biarkan tetap global)
        $stats = [
            'total' => Project::count(),
